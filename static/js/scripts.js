...
function TextAnalyzerEditor() {
    ...
    function load_results(type, val) {
        ...
        switch (type) {
            ...
            case 'summary':
                ...
                i = 0;
                while (i < val.length) {
                    w += "<li>" + val[i].trim() + '<a href="#" style="text-decoration:none" onclick="location.href = "http://stackoverflow.com/questions/3486110/make-a-list-item-clickable-html-css"; " id="' + val[i].trim() + '"></a></li>';
                    //w += "<li onclick="location.href = 'http://stackoverflow.com/questions/3486110/make-a-list-item-clickable-html-css';">" + val[i].trim() + '<a href="#" style="text-decoration:none" onclick="trigger(this)" id="' + val[i].trim() + '"></a></li>';
                  //w += '<li>' + val[i].trim() + '<a href="#" style="text-decoration:none" onclick="trigger(this)" id="' + val[i].trim() + '"><i style="margin-left: 0.5rem; vertical-align: middle; display: inline" </i></a></li>';
                  i += 1;
                }
                w = '' === w ? w : '<ol>' + w + '</ol>';
                ins.elem.boxes.summaryContent.innerHTML = w;
                break;
            ...
            case 'examples':
                ...
                w = '' === w ? w : '<ol>' + w + '</ol>';
                ins.elem.boxes.exampleContent.innerHTML = w;
                break;
            ...
            // case 'synset_list':
            //     ins.elem.boxes.synsetContent.innerHTML = val;
            //     break;
    ...
    function onclick_wordNETSentences(){
        ins.states.lemmatizedSentences = ! ins.states.lemmatizedSentences;
        load_results('sentences', lastResult[ ins.states.lemmatizedSentences ? 'lemmatized_sentences' : 'sentences' ] );
    }
    ...
    this.elem.boxes = {
        ...
        exampleContent: document.querySelector('.results-box.examples .rb-content'),
        ...
        // synsetContent: document.querySelector('.results-box synsets .rb-content')
    };
...
function mouseenter_tooltip_trigger(){
    ...
    // var $checkboxes = $("<div>one:<input id='Checkbox1' type='checkbox' /><br/>two:<input id='Checkbox1' type='checkbox' /></div>")
    //                  return $checkboxes;
    //
    // $("#example").popover({
    ...
    //table = table + "<tr><td>"+ content[i]+"</td><td>"+ content[i] +"</td></tr>";
    //}
    //table = table + "</table>";
    //return table;
...
function initTooltips( tt ){
    ...
    if( tt.length ){
        ...
        while (i < tt.length) {
            ...
            tt[i].addEventListener("mouseenter", mouseenter_tooltip_trigger); // this is the hover event
